...
describe("match controller - getAllMatches", () => {
  // dopo ogni test ripristino tutti gli stub per non influenzare i test successivi
  afterEach(() => {
    sinon.restore();
  });
  ...
  it("should return 200 if matches are found", async () => {
    ...
    // verifico che la risposta contenga matches
    const response = res.json.firstCall.args[0];
    expect(response).to.have.property(`matches`);
  });
  ...
  it("should return 500 if database throws an error", async () => {
    ...
    // stubbo getMatches per simulare un errore del database
    sinon.stub(matchModel, "getMatches").rejects(new Error(`database error`));
